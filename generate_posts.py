...
- **事実の厳守**: 産地・数字・固有名詞はこのブランド情報にある事実のみ使う。椨の産地は「福岡県産」までしか書かない（「八女」は杉の話であり椨には使わない誤り例）。白檀は輸入香木なので「国産白檀」と書かない。ここに無い事実は創作しない
...
- 本文は4〜6行・55〜90字を基本とする。説明を足したくなったら追いリプへ移す
- 本文内に、他人へ伝えたくなる確認済みの燃料を必ず1つ入れる（知られていない事実・具体的数字・失われる危機・意外な対比）。価値観と「そっといいね」だけで終わらせない
...
- いいね率は「見た人への共感」、再投稿・引用・シェア率は「配信拡大」と分けて評価する。いいね率だけでバズ確度を上げない
- CI実測（2026-09-12）: 白檀の価値観＋いいね依頼はいいね率11.75%でも再投稿率0.06%・閲覧3,217で停止。椨「あと2人」はいいね率12.8%・再投稿率0.17%・閲覧116,475。差は伝えたくなる具体的事実の有無
...
- confidenceは候補間の相対スコア0〜100であり、バズする確率ではない。確認済みの燃料がない投稿は79以下、同型で高い再投稿率を再現していない投稿は90以上にしない"""
